...
void initialize(void) {
	...
	initp.arg = -1;
	initp.next = NULL;
	initp.Period_Deadline = 80000;
	initp.Rel_Period_Deadline = 80000;	
 ...
 * implementation to enqueue with insertion sort.
 */
static void enqueueSort(thread p, thread *queue) {
	p->next = NULL;
	if (*queue == NULL) 
	{
		*queue = p;
	}
	else if(p->Rel_Period_Deadline < (*queue)->Rel_Period_Deadline)
	{
		p->next = *queue;
		*queue = p;
	}
...
void spawnWithDeadline(void (* function)(int), int arg, unsigned int deadline, unsigned int rel_deadline) {
	...
	//printf_at_seg(3, "D:%d T:%d", doneQ->Period_Deadline, ticks);
	if (setjmp(newp->context) == 1) {
		ENABLE();
		current->function(current->arg);
		DISABLE();
...
void scheduler(void)
{
	...
	scheduler_RM();				//Rate monotonic scheduler
}
